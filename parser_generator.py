...
def generate(syntax: list[str], semantics, debug: bool = False) -> set:
    ...
    terminals = set(
        token
            ...
            for token in split(pattern)
        if is_terminal(token)
        )

    operators = list(
        embed_nonterminal(rule) for rule, alternatives in rules.items()
            if any(t in split(alternative) for t in terminals for alternative in alternatives)
        )
...
def parse(expr: str) -> 'Rule':
    return {embed_nonterminal(list(rules.keys())[0])}(expr)
...
def split(expr: str, sep: list, reversed: bool = False) -> list[str]|None:
    ...
    if reversed:
        ...
        if x in expr:
            if x == "":
                if len(expr) > 1:
                    left, right = expr[0], split(expr[1:], sep, reversed)
                else:
                    return [expr]
            else:
                left, right = expr.split(x, 1)
                right = split(right, sep, reversed)

            return None if (right == None) else ([left] if left else []) + [x] + right
...
class {embed_nonterminal(rule)}(Rule):
    ...
    Rule: `{docstring}`
    '''
    def __new__(rule, expr: str):
        rule.name = f"<{{rule.__name__}}>"
"""
...
"""
        ...
        tokens = []
        pattern = split(expr, {[t for t in pattern if t in terminals]}, reversed={"True" if leftRecursive else "False"})
        {'print("split expr:", pattern)' if debug else ""}
        ...
        if not (pattern == None) and len(pattern) == {len(pattern)}:
            for token, test in zip(pattern, [{", ".join(embed_nonterminal(t) if is_nonterminal(t) else f"'{t}'" for t in pattern)}]):            
                if isinstance(test, str):
                    tokens.append(token == test) # or None 
                else:
            ...
            if all(tokens): 
                ...
                children = [t for t in tokens if isinstance(t, Rule)]
...
"""  if terminalShortcut else "") + f"""
        ...
        return {" or ".join(nonterminalShortcut) if nonterminalShortcut else ("print(f'Failed {rule}: Backtracking')" if debug else "None")}
"""
